...
contract Crowdsale is Owner{
    ...
    mapping(address => uint256) public hadSend;
    ...
    **/
    ...
    function sendTokenToBuyer(address buyerAddress) public{
        ...
        uint256 waitSend = amount * price * (1 ether - hadSend[buyerAddress]) / 1 ether;
        if(tokenReward.balanceOf(contractAddress) < waitSend || hadSend[buyerAddress] >= 1 ether){
            revert();
        }//合约token余额判断
        tokenReward.transfer(buyerAddress, waitSend);
        hadSend[buyerAddress] = 1 ether;
        FundTransfer(buyerAddress, waitSend/price, true);
    }

    function sendTokenToBuyerByPercent(address buyerAddress,uint256 percent) public{
        require(balanceOf[buyerAddress] > 0); //数量 > 0
        require(ownerCheckGoalReached && fundingGoalReached); //需要执行完 checkGoalReached()/ico成功
        if(percent > 1 ether || hadSend[buyerAddress] >= 1 ether || (hadSend[buyerAddress] + percent) > 1 ether){
            revert();
        }
        uint256 amount = balanceOf[buyerAddress];
        address contractAddress = this;
        var sendToken = amount * price * percent / 1 ether;
        if(tokenReward.balanceOf(contractAddress) < sendToken){
            revert();
        }//合约token余额判断
        hadSend[buyerAddress] += percent;
        tokenReward.transfer(buyerAddress, sendToken);
        FundTransfer(buyerAddress, sendToken/price, true);
    }
